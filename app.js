...
app.get("/buku", (req, res) => {
  db.query("SELECT * FROM buku", (err, result) => {
    if (err) {
      ...
      return res.status(500).json({ error: "Gagal mengambil data buku", detail: err.message });
    }
    res.json(result);
  });
});
...
app.get("/buku/:id", (req, res) => {
  ...
  db.query("SELECT * FROM buku WHERE id_buku=?", [id_buku], (err, result) => {
    if (err) return res.status(500).json({ error: "Server error", detail: err.message });
    if (result.length === 0) return res.status(404).json({ error: "Buku tidak ditemukan", id: id_buku });
    res.json(result[0]);
  });
});
...
app.post("/buku", (req, res) => {
  ...
  if (!judul || !penulis || tahun_terbit == null || stok == null) {
    return res.status(400).json({ error: "Field judul, penulis, tahun_terbit, dan stok wajib diisi" });
  }

  if (isNaN(tahun_terbit) || isNaN(stok)) {
    return res.status(400).json({ error: "tahun_terbit dan stok harus berupa angka" });
  }
  ...
  db.query(
    ...
    (err, result) => {
      if (err) return res.status(500).json({ error: "Gagal menyimpan buku", detail: err.message });
...
app.put("/buku/:id", (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      if (err) return res.status(500).json({ error: "Gagal update buku", detail: err.message });
      if (result.affectedRows === 0) return res.status(404).json({ error: "Buku tidak ditemukan", id: id_buku });
...
app.delete("/buku/:id", (req, res) => {
  ...
  db.query("DELETE FROM buku WHERE id_buku=?", [id_buku], (err, result) => {
    if (err) return res.status(500).json({ error: "Gagal menghapus buku", detail: err.message });
    if (result.affectedRows === 0) return res.status(404).json({ error: "Buku tidak ditemukan", id: id_buku });
